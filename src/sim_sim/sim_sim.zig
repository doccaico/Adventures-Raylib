    ...
    fn drawParticles(self: Self) void {
        for (0..gridWidth) |i| {
            for (0..gridHeight) |j| {
                ...
                rl.DrawRectangle(@intCast(i * cellSide), @intCast(j * cellSide), cellSide, cellSide, color);
            }
        }
    ...
    fn putParticles(self: *Self, x: usize, y: usize, brush_size: usize, selected_brush: CellType) void {
        ...
        while (i < brush_size) : (i += 1) {
            ...
            while (j < brush_size) : (j += 1) {
                ...
                if (ix >= 0 and ix < gridWidth and iy >= 0 and iy < gridHeight) {
        ...
        while (j != std.math.maxInt(usize)) : (j -%= 1) {
            ...
            for (0..gridWidth) |i| {
                ...
                switch (self.grid[i][j]) {
                    .gravel => {
                        if (j + 1 < gridHeight and self.grid[i][j + 1] == .acid) {
                            self.grid[i][j] = .smoke;
                            self.grid[i][j + 1] = .smoke;
                        } else if (j + 1 < gridHeight and (self.grid[i][j + 1] == .air or self.grid[i][j + 1] == .water)) {
                    ...
                    .water => {
                        if (j + 1 < gridHeight and self.grid[i][j + 1] == .acid) {
                            self.grid[i][j] = .smoke;
                            self.grid[i][j + 1] = .smoke;
                        } else if (j + 1 < gridHeight and self.grid[i][j + 1] == .air) {
                            self.grid[i][j] = .air;
                    ...
                    .acid => {
                        if (j + 1 < gridHeight and (self.grid[i][j + 1] == .acid or self.grid[i][j + 1] == .rock)) {
                            continue;
                        } else if (j + 1 < gridHeight and self.grid[i][j + 1] != .air and self.grid[i][j + 1] != .smoke) {
                            self.grid[i][j] = .smoke;
                            self.grid[i][j + 1] = .smoke;
                ...
                switch (self.grid[i][j]) {
                    .water => {
                        if (@rem(rand.int(i32), 2) == 0) {
                            if (i != 0) {
                                if (i - 1 >= 0 and self.grid[i - 1][j] == .air) {
                                    self.grid[i - 1][j] = .water;
                                    self.grid[i][j] = .air;
                                }
                            }
                        } else {
                            if (i + 1 < gridWidth and self.grid[i + 1][j] == .air) {
                                self.grid[i + 1][j] = .water;
                                self.grid[i][j] = .air;
                            }
                        }
                    ...
                    .sand => {
                        if (@rem(rand.int(i32), 2) == 0) {
                            if (i != 0) {
                                if (i - 1 >= 0 and j + 1 < gridHeight and (self.grid[i - 1][j + 1] == .air or self.grid[i - 1][j + 1] == .water) and self.grid[i - 1][j] != .sand and self.grid[i - 1][j] != .gravel and self.grid[i - 1][j] != .rock and self.grid[i][j + 1] != .air) {
                        ...
                        } else if (i + 1 < gridWidth and j + 1 < gridHeight and (self.grid[i + 1][j + 1] == .air or self.grid[i + 1][j + 1] == .water) and self.grid[i + 1][j] != .sand and self.grid[i + 1][j] != .gravel and self.grid[i + 1][j] != .rock and self.grid[i][j + 1] != .air) {
                            self.grid[i][j] = self.grid[i + 1][j];
                            moved[i][j] = true;
                            self.grid[i + 1][j] = .sand;
                            moved[i + 1][j] = true;
                    ...
                    .smoke => {
                        ...
                        } else if (i + 1 < gridWidth and self.grid[i + 1][j] == .air) {
                            self.grid[i + 1][j] = .smoke;
                            self.grid[i][j] = .air;
                        }
                    },
                    .acid => {
                        if (@rem(rand.int(i32), 2) == 0) {
                            if (i != 0) {
                                if (j + 1 < gridHeight and i - 1 >= 0 and self.grid[i][j + 1] == .acid and self.grid[i - 1][j] != .air and self.grid[i - 1][j] != .acid and self.grid[i - 1][j] != .rock) {
                                    self.grid[i - 1][j] = .smoke;
                                    self.grid[i][j] = .smoke;
                                } else if (i - 1 >= 0 and self.grid[i - 1][j] == .air) {
                                    self.grid[i][j] = self.grid[i - 1][j];
                                    self.grid[i - 1][j] = .acid;
                                }
                            }
                        } else {
                            if (j + 1 < gridHeight and i + 1 < gridWidth and self.grid[i][j + 1] == .acid and self.grid[i + 1][j] != .air and self.grid[i + 1][j] != .acid and self.grid[i + 1][j] != .rock) {
                                self.grid[i + 1][j] = .smoke;
                                self.grid[i][j] = .smoke;
                            } else if (i + 1 < gridWidth and self.grid[i + 1][j] == .air) {
                                self.grid[i][j] = self.grid[i + 1][j];
